...
pub async fn scrim_ban(
    ...
    embed
        .set_author(author.name(format!("{} Scrim Banned", to_ban.name)).clone())
        .field("User", to_ban.mention().to_string(), true)
        .color(0xFD4659);
    ...
    let _ = CONFIG
        ...
        .send_message(&ctx, |msg| {
            msg.set_embed(embed.clone().fields(fields.clone()).clone())
        })
        .await
        .map_err(|err| tracing::error!("Failed to log to support bans: {}", err));
    ...
    let _ = CONFIG
        ...
        .await
        .map_err(|err| tracing::error!("Failed to log to public bans: {}", err));
